...
export abstract class WeekUnit extends MonthUnit {
...
export abstract class SecondUnit extends DayUnit {
...
export class LunarWeek extends WeekUnit {
    ...
    protected constructor(year: number | string, month: number | string, index: number | string, start: number | string) {
        ...
        const i: number = SolarWeek.numeric(index, 'lunar week index');
        const s: number = SolarWeek.numeric(start, 'week index');
...
export class LunarDay extends DayUnit {
    ...
    protected constructor(year: number | string, month: number | string, day: number | string) {
        const y: number = LunarDay.numeric(year, 'lunar year');
        const m: number = LunarDay.numeric(month, 'lunar month');
...
export class LunarHour extends SecondUnit {
    ...
    protected constructor(year: number | string, month: number | string, day: number | string, hour: number | string, minute: number | string, second: number | string) {
        const y: number = LunarHour.numeric(year, 'lunar year');
        const m: number = LunarHour.numeric(month, 'lunar month');
        const d: number = LunarHour.numeric(day, 'lunar day');
        const h: number = LunarHour.numeric(hour, 'hour');
        const n: number = LunarHour.numeric(minute, 'minute');
    ...
    getSixtyCycle(): SixtyCycle {
        let e: number = this.getIndexInDay();
        let h: HeavenStem = this.getLunarDay().getSixtyCycle().getHeavenStem();
        if (this.hour >= 23) {
            h = h.next(1);
            e = 0;
        }
    ...
    getNineStar(): NineStar {
        const d: LunarDay = this.getLunarDay();
        const solar: SolarDay = d.getSolarDay();
        const dongZhi: SolarTerm = SolarTerm.fromIndex(solar.getYear(), 0);
        const earthBranchIndex: number = this.getIndexInDay() % 12;
        let index: number = [8, 5, 2][d.getSixtyCycle().getEarthBranch().getIndex() % 3];
        if (!solar.isBefore(dongZhi.getJulianDay().getSolarDay()) && solar.isBefore(dongZhi.next(12).getJulianDay().getSolarDay())) {
            index = 8 + earthBranchIndex - index;
        } else {
            index -= earthBranchIndex;
...
export class SolarTerm extends LoopTyme {
    ...
    protected constructor(year: number | string, indexOrName: number | string) {
        ...
        let y: number = SolarTerm.numeric(year, 'solar term year');
        if (typeof indexOrName === 'number') {
...
export class SolarHalfYear extends YearUnit {
    static NAMES: string[] = ['上半年', '下半年'];
    protected index: number;

    protected constructor(year: number | string, index: number | string) {
        const y: number = SolarHalfYear.numeric(year, 'solar year');
...
export class SolarSeason extends YearUnit {
    static NAMES: string[] = ['一季度', '二季度', '三季度', '四季度'];
    protected index: number;

    protected constructor(year: number | string, index: number | string) {
        const y: number = SolarSeason.numeric(year, 'solar year');
...
export class SolarDay extends DayUnit {
    ...
    protected constructor(year: number | string, month: number | string, day: number | string) {
        const y: number = SolarDay.numeric(year, 'solar year');
        const m: number = SolarDay.numeric(month, 'solar month');
    ...
    getDogDay(): DogDay | null {
        // 初伏，夏至后第3个庚日
        const d0: SolarDay | null = Event.builder().termHeavenStem(12, 6, 20).build().getSolarDay(this.year);
        // 中伏，夏至后第4个庚日
...
export class SolarTime extends SecondUnit {
    protected constructor(year: number | string, month: number | string, day: number | string, hour: number | string, minute: number | string, second: number | string) {
        const y: number = SolarTime.numeric(year, 'solar year');
        const m: number = SolarTime.numeric(month, 'solar month');
        const d: number = SolarTime.numeric(day, 'solar day');
        const h: number = SolarTime.numeric(hour, 'hour');
        const n: number = SolarTime.numeric(minute, 'minute');
...
export class LegalHoliday extends AbstractTyme {
    static NAMES: string[] = ['元旦', '春节', '清明节', '劳动节', '端午节', '中秋节', '国庆节', '国庆中秋', '抗战胜利日'];
...
export class LunarFestival extends AbstractTyme {
    ...
    static fromYmd(year: number, month: number, day: number): LunarFestival | null {
        ...
        while (matcher = reg.exec(LunarFestival.DATA)) {
            const data: string = matcher[0];
            const term: SolarTerm = SolarTerm.fromIndex(year, parseInt(data.substring(4), 10));
            const termDay: SolarDay = term.getSolarDay();
...
export class RabByungMonth extends MonthUnit {
    ...
    protected static init(): void {
        ...
        for (let x: number = 0, z: number = years.length; x < z; x++) {
    ...
    public constructor(year: number | string, month: number | string) {
        RabByungMonth.init();
    ...
    getIndexInYear(): number {
        ...
        if (this.leap) {
            ...
            const leapMonth: number = this.getRabByungYear().getLeapMonth();
    ...
    getLeapDays(): number[] {
        ...
        for (let i: number = 0, j: number = days.length; i < j; i++) {
...
export class RabByungDay extends DayUnit {
    ...
    protected constructor(year: number | string, month: number | string, day: number | string) {
        const y: number = RabByungDay.numeric(year, 'rab-byung year');
        const m: number = RabByungDay.numeric(month, 'rab-byung month');
        const d: number = RabByungDay.numeric(day, 'rab-byung day');
    ...
    static fromSolarDay(solarDay: SolarDay): RabByungDay {
        ...
        for (let i: number = 0, j: number = specialDays.length; i < j; i++) {
    ...
    getSolarDay(): SolarDay {
        let m: RabByungMonth = RabByungMonth.fromYm(1950, 12);
        const cm: RabByungMonth = this.getRabByungMonth();
        let n: number = 0;
        while (!m.equals(cm)) {
            n += m.getDayCount();
            m = m.next(1);
...
export class Event extends AbstractCulture {
    protected name: string;
    ...
    static fromName(name: string): Event | null {
        const matcher: RegExpExecArray | null = new RegExp(EventManager.REGEX + `(${name})`, 'g').exec(EventManager.DATA);
    ...
    getStartYear(): number {
        let n: number = 0;
        const size: number = EventManager.CHARS.length;
    ...
    static fromSolarDay(d: SolarDay): Event[] {
        const l: Event[] = [];
        const events: Event[] = Event.all();
        for (let i: number = 0, j: number = events.length; i < j; i++) {
            const e: Event = events[i];
            const day: SolarDay | null = e.getSolarDay(d.getYear());
            if (day && d.equals(day)) {
                l.push(e);
            }
        }
    ...
    static all(): Event[] {
        const l: Event[] = [];
        const reg: RegExp = new RegExp(EventManager.REGEX + `(.[^@]+)`, 'g');
        let matcher: RegExpExecArray | null;
    ...
    getSolarDay(year: number): SolarDay | null {
        ...
        const offset: number = EventManager.CHARS.indexOf(this.data.charAt(5)) - 31;
        return 0 == offset ? d : d.next(offset);
    }

    getSolarDayBySolarDay(year: number): SolarDay | null {
        let y: number = year;
        let m: number = EventManager.CHARS.indexOf(this.data.charAt(2)) - 31;
        if (m > 12) {
            m = 1;
            y += 1;
        }
        const d: number = EventManager.CHARS.indexOf(this.data.charAt(3)) - 31;
        const delay: number = EventManager.CHARS.indexOf(this.data.charAt(4)) - 31;
        const month: SolarMonth = SolarMonth.fromYm(y, m);
        const lastDay: number = month.getDayCount();
        if (d > lastDay) {
            if (0 == delay) {
                return null;
            }
    ...
    getSolarDayByLunarDay(year: number): SolarDay | null {
        ...
        const d: number = EventManager.CHARS.indexOf(this.data.charAt(3)) - 31;
        const delay: number = EventManager.CHARS.indexOf(this.data.charAt(4)) - 31;
        const month: LunarMonth = LunarMonth.fromYm(y, m);
        const lastDay: number = month.getDayCount();
        if (d > lastDay) {
            if (0 == delay) {
                return null;
    ...
    getSolarDayByWeek(year: number): SolarDay | null {
        // 第几个星期
        const n: number = EventManager.CHARS.indexOf(this.data.charAt(3)) - 31;
        if (n === 0) {
            return null;
        }
        const m: SolarMonth = SolarMonth.fromYm(year, EventManager.CHARS.indexOf(this.data.charAt(2)) - 31);
        // 星期几
        const w: number = EventManager.CHARS.indexOf(this.data.charAt(4)) - 31;
        if (n > 0) {
            // 当月第1天
            const d: SolarDay = m.getFirstDay();
            // 往后找第几个星期几
            return d.next(d.getWeek().stepsTo(w) + 7 * n - 7);
        }
        // 当月最后一天
        const d: SolarDay = SolarDay.fromYmd(year, m.getMonth(), m.getDayCount());
        // 往前找第几个星期几
        return d.next(d.getWeek().stepsBackTo(w) + 7 * n + 7);
    }

    getSolarDayByTerm(year: number): SolarDay {
        const offset: number = EventManager.CHARS.indexOf(this.data.charAt(4)) - 31;
        const d: SolarDay = SolarTerm.fromIndex(year, EventManager.CHARS.indexOf(this.data.charAt(2)) - 31).getSolarDay();
        return 0 == offset ? d : d.next(offset);
    }

    getSolarDayByTermHeavenStem(year: number): SolarDay | null {
        const d: SolarDay = this.getSolarDayByTerm(year);
...
class EventBuilder {
    ...
    static encodeType(t: EventType): string {
        let n: number = 0;
        switch (t) {
            case EventType.SOLAR_WEEK:
                n = 1;
...
export class EventManager {
    static REGEX: string = '(@[0-9A-Za-z_]{8})';
    static DATA: string = '';
